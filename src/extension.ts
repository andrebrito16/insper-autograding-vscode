...
import { GradesPanel } from "./app/panels/grades-panel";
import { upsertMarkdownCellBellow } from "./app/utils/upsert-cell";

export async function activate(context: vscode.ExtensionContext) {
	console.log(
		'Congratulations, your extension "insper-autograding-new" is now active!',
	);
	...
	if (!authData) {
		// setupToken(); Disable for development
	}

	const disposable = vscode.commands.registerCommand(
		"insper-autograding.helloWorld",
		() => {
			vscode.window.showInformationMessage(
				"Hello World from insper-autograding-new!",
			);
		},
	);
		...
		async ({
			...
			if (cell.metadata.metadata.iag === "submit") {
				...
				if (match) {
					...
					vscode.window.showInformationMessage(
						`Valor da variável ${variableName}: ${value}`,
					);
				} else {
					vscode.window.showErrorMessage(
						`Variável ${variableName} não encontrada`,
					);
				}
			} else {
	...
	const webview = vscode.commands.registerCommand(
		"insper-autograding.webview",
		() => {
			GradesPanel.render(context.extensionUri);
